...
/*   Updated: 2024/02/23 16:12:05 by davifern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_philo	*create_philo(char **argv)
{
	t_philo	*philo;
...
void	*routine(void *god_data)
{
	...
	while (!philosopher_died(philo->fasting, philo->now, philo->time_to_die)) //passar philo depois
	{
		...
		printf("%.5lu X has taken a fork\n", get_current_time(god->start, philo->now));
		...
		gettimeofday(&philo->now, NULL);
		printf("%.5lu X is thinking\n", get_current_time(god->start, philo->now)); //pensa
	}
	printf("%.5lu X died\n", get_current_time(god->start, philo->now)); //morre
...
int	main(int argc, char **argv)
{
	...
	god = create_god();
	philo = create_philo(argv);
	god->philo = philo;
	
	
	pthread_create(&tid1, NULL, routine, (void *)god);
